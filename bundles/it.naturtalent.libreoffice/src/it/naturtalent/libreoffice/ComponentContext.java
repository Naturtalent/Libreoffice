...
public class ComponentContext implements XComponentContext, XComponent
{
	...
	public ComponentContext(Hashtable<String, Object> table,
	...
	/**
	 * Ctor to create a component context passing a hashtable for values and a
	 * delegator reference. Entries of the passed hashtable are either direct
	 * values or ComponentContextEntry objects.
	 * 
	 * @param table
	 *            entries
	 * @param xDelegate
			...
			XComponentContext xDelegate)
	{
		...
		if (o != null)
		{
			...
			m_xSMgr = UnoRuntime
					.queryInterface(XMultiComponentFactory.class, o);
		}
		if (m_xSMgr != null)
		{
			m_bDisposeSMgr = true;
				...
				if (entry.m_lateInit != null)
				{
					...
					try
					{
						...
						if (serviceName != null)
						{
							if (m_xSMgr != null)
							{
								xInstance = m_xSMgr.createInstanceWithContext(
										serviceName, this);
							}
							else
							{
								if (DEBUG)
									System.err
											.println("### no service manager instance for late init of singleton instance \""
													+ rName + "\"!");
							}
						}
						else
						{
							XSingleComponentFactory xCompFac = UnoRuntime
									.queryInterface(
							...
							else
							{
								if (DEBUG)
									System.err
											.println("### neither service name nor service factory given for late init of singleton instance \""
													+ rName + "\"!");
							}
						}
					} catch (com.sun.star.uno.Exception exc)
					{
						if (DEBUG)
							System.err
									.println("### exception occurred on late init of singleton instance \""
											+ rName + "\": " + exc.getMessage());
					}
					if (xInstance != null)
					{
						synchronized (entry)
					...
					else
					{
						if (DEBUG)
							System.err
									.println("### failed late init of singleton instance \""
											+ rName + "\"!");
					}
				}
