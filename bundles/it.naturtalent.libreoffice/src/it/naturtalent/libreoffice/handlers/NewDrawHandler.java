...
public class NewDrawHandler
{
	...
	public void execute(IEventBroker eventBroker, @Named(IServiceConstants.ACTIVE_SHELL) Shell shell)
	{
		...
		if (selObject instanceof IResource)
		{
			...
			// neue Datei erzeugen
			String newName = getAutoFileName((IContainer) iResource, DRAW_FILENAME);				
			...
			new DrawDocument().loadPage(path.toString());
		}
	}
	...
	public static void createDrawFile(File destDrawFile)
	{
		...
		URL urlTemplate = FileLocator.find(bundleContext.getBundle(),new Path(DRAW_TEMPLATE), null);
		try
		{
