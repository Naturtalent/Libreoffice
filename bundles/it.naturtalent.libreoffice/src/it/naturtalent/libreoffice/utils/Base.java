...
public class Base
{
	...
	public static XOfficeDatabaseDocument createBaseDoc(String fnm,
			XComponentLoader loader)
	{
		return createBaseDoc(fnm, Base.HSQLDB, loader);
	}

	public static XOfficeDatabaseDocument createBaseDoc(String fnm, int dbType,
			XComponentLoader loader)
	{
		if ((dbType != HSQLDB) && (dbType != FIREBIRD))
		{
			System.out.println(
					"Did not recognize the database type; using HSQLDB");
			dbType = HSQLDB;
		}
		...
		XOfficeDatabaseDocument dbDoc = Lo.qi(XOfficeDatabaseDocument.class,
				doc);
		...
		System.out.println(
				"Created a \"" + sourceStr + "\" Base document in " + fnm);
	...
	public static void closeBaseDoc(XOfficeDatabaseDocument dbDoc)
	{
		try
		{
			com.sun.star.util.XCloseable closeable = Lo
					.qi(com.sun.star.util.XCloseable.class, dbDoc);
			Lo.close(closeable);
		} catch (com.sun.star.lang.DisposedException e)
		{
			System.out.println(
					"Database close failed since Office link disposed");
		}
	} // end of closeBaseDoc()

	public static XOfficeDatabaseDocument openBaseDoc(String fnm,
			XComponentLoader loader)
	{
	...
	public static void refreshTables(XConnection conn)
	{
		...
		XRefreshable refresh = Lo.qi(XRefreshable.class,
				suppTables.getTables());
		refresh.refresh();
	} // end of refreshTables()

	public static void showTables(XOfficeDatabaseDocument dbDoc)
	/*
	 * This way of showing tables often causes Office to crash when Office is
	 * closed while the table windows are open on-screen.
	 * 
	...
	public static XController loadTable(XOfficeDatabaseDocument dbDoc,
			String tableName)
	{
		try
		{
			...
			XComponent tableComp = docUI.loadComponent(DatabaseObject.TABLE,
					tableName, false);
	...
	public static void showTable(XOfficeDatabaseDocument dbDoc,
			String tableName)
	{
		XController ctrl = loadTable(dbDoc, tableName);
		if (ctrl != null)
		{
			XFormController tableViewController = Lo.qi(XFormController.class,
					ctrl);
	...
	public static void displayTable(String tableName, XConnection conn,
			int offset)
	{
		try
		{
			XStatement statement = conn.createStatement();
			XResultSet rs = statement
	...
	public static int getColumnCount(XResultSet rs)
	{
		...
		try
		{
			XResultSetMetaDataSupplier rsMetaSupp = Lo
					.qi(XResultSetMetaDataSupplier.class, rs);
	...
	public static void displayResultSet(XResultSet rs, final String title,
			final int offset)
	{
		if (rs == null)
		{
		...
		try
		{
			...
			SwingUtilities.invokeLater(new Runnable()
			{
				public void run()
				{
					...
					frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE); // so
																				// don't
																				// kill
																				// main
	...
	private static Object[] getRow(XRow xRow, int numCols) throws SQLException
	...
	public static Object[][] getResultSetArr(XResultSet rs)
	{
		...
		System.out
				.println("Result set array size: " + numRows + " x " + numCols);
	...
	public static ArrayList<String> readCmds(String fnm)
	{
		...
		try
		{
			...
			while ((line = br.readLine()) != null)
			{
				...
				else if (line.startsWith("CREATE") || // start of new command
														// line
	...
	/*
	 * return the name of the embedded database stored in fnm by looking inside
	 * its content.xml file
	 */
	{
		...
		try
		{
			...
			while (zipEntries.hasMoreElements())
			{
				...
				if (zipEntry.getName().startsWith("content.xml"))
					foundContent = Base.writeToFile(contentFnm, zipFile,
							zipEntry);
			}
			zipFile.close();
		} catch (java.lang.Exception e)
	...
	public static boolean writeToFile(String fnm, ZipFile zipFile,
			ZipEntry zipEntry)
	...
	/*
	 * A ODB file using an embedded database will have a db:connection-resource
	 * xlink:href attribute stored in its content.xml file, fnm
	 */
	{
		...
		try
		{
			...
			XPathExpression expr = xpath
					.compile("//db:connection-resource/@xlink:href");
	...
	/*
	 * return the name of the linked database named in fnm by looking inside its
	 * content.xml file
	 */
	{
		FileIO.makeDirectory(TEMP_DIR);
	...
	/*
	 * A ODB file that links to a database will have a db:file-based-database
	 * xlink:href attribute stored in its content.xml file, fnm
	 */
	{
		DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
		...
		try
		{
			...
			XPathExpression expr = xpath
					.compile("//db:file-based-database/@xlink:href");
			resource = (String) expr.evaluate(doc, XPathConstants.STRING);
			if ((resource == null) || resource.equals(""))
			{
	...
	public static ArrayList<String> extractEmbedded(String fnm)
	{
		...
		{
			System.out.println(
					fnm + " is not an embedded HSQL/Firebird database");
			return null;
		}
		...
		{
			System.out.println(
					fnm + " holds an unknown embedded database: " + embedFnm);
			return null;
		}
	...
	private static String storeFile(ZipFile zipFile, ZipEntry zipEntry,
			int embedType)
	{
	...
	/*
	 * public static void fixOwnership(String fnm) // fix Firebird's 2.5.5.
	 * ownership problem by calling fbfix.bat // on fnm in TEMP_DIR { try {
	 * Runtime.getRuntime().exec("cmd /c fbfix.bat " + fnm); } catch
	 * (java.lang.Exception e) {
	 * System.out.println("Unable to fix Firebird ownership: " + e); } } // end
	 * of fixOwnership()
	 */
	...
	public static XRowSet rowSetQuery(String fnm, String query)
	{
		...
		Props.setProperty(xRowSet, "CommandType", CommandType.COMMAND); // TABLE,
																		// QUERY
																		// or
																		// COMMAND
		Props.setProperty(xRowSet, "Command", query);
		// command could be a table, query name, or SQL, depending on the
		// CommandType
	...
	public static void displayTableInfo(XConnection conn, String tableNm)
	{
		...
		try
		{
			displayTableProperties(tables.getByName(tableNm)); // a sdbcx Table
																// service
		} catch (com.sun.star.uno.Exception e)
		{
	...
	// print attributes stored in sdbcx Table service
	{
		...
		System.out.println(
				"  Name: \"" + Props.getProperty(table, "Name") + "\"");
		System.out.println("  Catalog Name: \""
				+ Props.getProperty(table, "CatalogName") + "\"");
		System.out.println("  Schema Name: \""
				+ Props.getProperty(table, "SchemaName") + "\"");
		System.out.println("  Description: \""
				+ Props.getProperty(table, "Description") + "\"");

		// the following property is optional so we first must check if it
		// exists
		if (Props.hasProperty(table, "Type"))
			System.out.println(
					"  Type: \"" + Props.getProperty(table, "Type") + "\"");
	...
	// print all columns of a XColumnsSupplier
	{
		...
		XNameAccess columns = colsSupplier.getColumns(); // a sdbcx Column
															// service
			...
			for (int i = 0; i < keys.getCount(); i++)
			{
				try
				{
					...
					if (key != null)
					{
						...
						XColumnsSupplier keyColumnsSup = Lo
								.qi(XColumnsSupplier.class, key);
	...
	// examine sdbcx Key service properties
	{
		...
		System.out.print(" ReferencedTable: "
				+ Props.getProperty(key, "ReferencedTable"));
		System.out
				.print(" UpdateRule: " + Props.getProperty(key, "UpdateRule"));
		System.out
				.print(" DeleteRule: " + Props.getProperty(key, "DeleteRule"));
		// System.out.println();
		...
		if (xIndexesSup != null)
		{
			...
			for (int i = 0; i < indexNms.length; i++)
			{
				...
				try
				{
					...
					XColumnsSupplier indexColumnsSup = Lo
							.qi(XColumnsSupplier.class, index);
	...
	{
		System.out.print(
				"  -- Name: \"" + Props.getProperty(column, "Name") + "\"");
		System.out.print("  Type: " + Props.getProperty(column, "Type"));
		System.out
				.print("  TypeName: " + Props.getProperty(column, "TypeName"));
		System.out.print(
				"  Precision: " + Props.getProperty(column, "Precision"));
		System.out.print("  Scale: " + Props.getProperty(column, "Scale"));
		System.out.print(
				"  IsNullable: " + Props.getProperty(column, "IsNullable"));
		System.out.print("  IsAutoIncrement: "
				+ Props.getProperty(column, "IsAutoIncrement"));
		System.out.print(
				"  IsCurrency: " + Props.getProperty(column, "IsCurrency"));

		// the following properties are optional
		if (Props.hasProperty(column, "IsRowVersion"))
			System.out.print("  IsRowVersion: "
					+ Props.getProperty(column, "IsRowVersion"));

		if (Props.hasProperty(column, "Description"))
			System.out.print("  Description: "
					+ Props.getProperty(column, "Description"));

		if (Props.hasProperty(column, "DefaultValue"))
			System.out.print("  DefaultValue: "
					+ Props.getProperty(column, "DefaultValue"));
	...
	// examine sdbcx Index service properties
	{
		...
		System.out.print(" IsPrimaryKeyIndex: "
				+ Props.getProperty(index, "IsPrimaryKeyIndex"));
		System.out.print(
				" IsClustered: " + Props.getProperty(index, "IsClustered"));
		System.out.println();
	} // end of printIndexProperties()

	public static void printGroups(XConnection conn)
	// print all groups and the users with their privileges who belong to this
	// group
	// examine sdbcx Group and User services properties
	{
		...
		for (int i = 0; i < groupNms.length; i++)
		{
			...
			try
			{
				XUsersSupplier usersSupp = Lo.qi(XUsersSupplier.class,
						xGroups.getByName(groupNms[i]));
				if (usersSupp != null)
				{
					...
					for (int j = 0; j < userNms.length; j++)
						System.out
								.println("\t    " + userNms[j] + " Privileges: "
										+ auth.getPrivileges(userNms[j],
	...
	public static void reportDBInfo(XConnection conn)
	{
		try
		{
			...
			if ((productName == null) || productName.equals(""))
				...
				System.out.println(
						"DB:  " + productName + " v." + productVersion);
			...
			if ((driverName == null) || driverName.equals(""))
				...
				System.out.println(
						"SDBC driver:  " + driverName + " v." + driverVersion);
		} catch (SQLException e)
		{
			System.out.println(e);
		}
	...
	// get table names using DatabaseMetaData
	{
		...
		try
		{
			...
			XResultSet rs = dm.getTables(null, null, "%", new String[]
				{ "TABLE" });
			XRow xRow = Lo.qi(XRow.class, rs);
			while (rs.next())
	...
	public static ArrayList<String> getColumnNames(XConnection conn,
			String tableName)
	{
		ArrayList<String> names = new ArrayList<String>();
		try
	...
	private static void printTypeConcurrency(XDatabaseMetaData md, int t,
			String typeStr) throws SQLException
	/*
	 * Possible ResultSet Type Values: FORWARD_ONLY: The result set is not
	 * scrollable (default) SCROLL_INSENSITIVE: The result set is scrollable but
	 * not sensitive to database changes. SCROLL_SENSITIVE: The result set is
	 * scrollable and sensitive to database changes.
	 * 
	 * Possible 'Concurrency' Values: READ_ONLY: The result set cannot be used
	 * to update the database (default) UPDATABLE: The result set can be used to
	 * update the database.
	 */
	{
		if (md.supportsResultSetType(t))
		{
			...
			if (md.supportsResultSetConcurrency(t,
	...
	{
		XDatabaseRegistrations dbRegs = Lo.createInstanceMCF(
				XDatabaseRegistrations.class,
			...
			Object dataSource)
	{
		...
		{ // Register it with the database context
			try
			{
				XNamingService nmService = Lo.createInstanceMCF(
			...
			{
				System.out
						.println("Unable to access data source naming service");
			}
		}
	...
	public static Object createDataSource()
	{
		try
		{
			XSingleServiceFactory factory = Lo.createInstanceMCF(
	...
	public static ArrayList<String> getSupportedDrivers()
	{
		...
		while (driversEnum.hasMoreElements())
		{
			...
			{
				drivers.add(
						Info.getImplementationName(driversEnum.nextElement()));
			} catch (com.sun.star.uno.Exception e)
			{
			}
		}
	...
	public static void printDriverProperties(XDriver driver, String url)
	{
		...
		{
			System.out.println(
					"Driver Name: " + Info.getImplementationName(driver));
